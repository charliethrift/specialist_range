...
fam_hill_2_s_chr_df <- read.csv("../data/phylo_numbers_df.csv")
fam_hill_2_s_chr_df$eoo_size_km2 <- fam_hill_2_s_chr_df$eoo_size_m2/1e6

glm_phylo <- glmmTMB::glmmTMB(eoo_size_km2 ~ fam_hill_2 + bee_family, 
                              data = fam_hill_2_s_chr_df, family = Gamma(link = "log"))
summary(glm_phylo)
tidy_mod_phylo <- broom.mixed::tidy(glm_phylo, effects = "fixed", conf.int = TRUE)
ft_phylo <- flextable::as_flextable(tidy_mod_phylo,random = TRUE)
ft_phylo
tidy_mod_phylo
...
glm_phylo15 <- glmmTMB::glmmTMB(eoo_size_km2 ~ fam_hill_2 + bee_family, 
                 data = sdfp, family = Gamma(link = "log"))
summary(glm_phylo15)
tidy_mod_phylo_15 <- broom.mixed::tidy(glm_phylo15, effects = "fixed", conf.int = TRUE)
...
#summary(mod_gen_phy)
glm_genus <- glmmTMB::glmmTMB(eoo_size_km2 ~ genus_hill + bee_family, 
                                data = genus_data_scores_2, family = Gamma(link = "log"))
summary(glm_genus)
tidy_mod_genus <- broom.mixed::tidy(glm_genus, effects = "fixed", conf.int = TRUE)
...
glm_fam174 <- glmmTMB::glmmTMB(eoo_size_km2 ~ fam_hill_2 + bee_family, 
                              data = fam_filtered, family = Gamma(link = "log"))
summary(glm_fam174)
tidy_mod_fam174 <- broom.mixed::tidy(glm_fam174, effects = "fixed", conf.int = TRUE)
